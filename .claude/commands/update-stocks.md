...
4. **이력** — `data/stocks-update.js` 맨 앞에 엔트리 prepend.
   ...
   - 동일 날짜 누적 원칙 (덮어쓰기 금지)
   - 🔴 **현재가·등락률은 라이브 시세 대조 필수**: summary 첫 줄 대표 종목의 가격·등락률·방향(±)을 편집 직전 라이브 시세로 교차검증해 일치시킨다. **자동 sandbox는 Supabase 피드가 차단되므로 웹검색으로**(예 `TICKER stock price`), 로컬 실행은 `prices/latest.json` 피드로. ±3% 이상 벌어지거나 부호가 반대면 라이브 값 우선. 자세한 건 CLAUDE.md [현재가·등락률은 라이브 피드와 대조] 참조.
