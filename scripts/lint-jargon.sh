...
PATTERNS='비트|미스|인라인|rs 갱신|rs 및|rs 맥락|실적 반영|수치 반영|갱신 반영|데이터 반영|rs 반영|도브 톤|매파 톤|약세 톤|회복 톤|디에스컬레이션 톤|거부 톤|우호 톤|\bATH\b|솼'
EXCLUDE='비트코인|비트맥스|비트파|비트 디지털|비트디어|비트팜|큐비트|온프레미스|프리미스|미스트랄|메가트론|선반영|priced in|MATH|PATH'
