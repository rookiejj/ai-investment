...
`summary`는 매일 평일 KST 08:00 친구톡에 **한 탭 한 줄(글머리표 1개)로 박힌다**. daily-send은 summary 텍스트 그대로 출력한다 (자동 자르기·분할 없음).

- **🔴 탭당 summary 길이 ≤ 150자 (엄수)**. 6탭 × 150자 + 헤더/구분자 ≈ 960자로 친구톡 1000자 한도 마진 확보. 초과 시 SOLAPI가 발송 거부할 수 있다.
- **자유 문체**: `·`·`,` 등 한국어 자연스러운 구분자 자유 사용.
- **사용자향 자연어**: 위 [update.js 사용자향 문체 규칙] 그대로 적용. 내부 필드명·메타 표현 금지.

**150자 가늠 팁**: 한국어 한 문장(주술 1세트, 부가 1~2개) ≈ 50~70자. 한 탭 summary는 짧은 문장 2~3개 분량. "오늘 핵심 1가지 + 보조 1~2가지"로 압축.
