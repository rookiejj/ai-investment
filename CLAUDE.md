...
**🔴 파이프라인 보호 (엄수)**: weekly-insight 작성에 실패하거나 좋은 주제를 찾지 못하면 **이 단계를 skip하고 메인 데이터 파일만 커밋한다**. weekly-insight.js가 메인 커밋을 막아서는 절대 안 된다. 인스타 포스팅 트리거까지 이어지는 파이프라인이 이 파일 하나 때문에 끊기면 안 된다.
...
1. **이번 주 움직임이 트리거** — 5탭 데이터 조사 단계에서 발견한 가장 큰 cross-asset 이벤트(지수 급등락·금리 급변·환율 이벤트·원자재 스파이크)를 주제의 씨앗으로 삼는다. "왜 그 자산이 그렇게 움직였나"에서 출발한다.
2. **비직관적 연결** — "A가 오르면 B도 오른다" 수준의 상관관계가 아니라, "왜?"라는 질문이 필요한 인과 메커니즘이 있어야 한다. 독자가 "몰랐네"라고 반응할 만큼 의외여야 한다.
3. **한국 투자자 관련성** — 코스피·코스닥·원달러 환율과 연결지어 생각할 수 있어야 한다. 미국만의 이야기보다 한국 포트폴리오에 영향이 오는 경로가 있는 것.
4. **설명 가능성** — 4~6개 인과 단계로 명확하게 설명 가능한 것. 설명이 너무 복잡하거나 예외가 너무 많은 메커니즘은 피한다.
5. **지난 4주 비중복** — 현재 `week` 필드를 읽어 최근 주제 확인. 비슷한 메커니즘(금리·할인율 계열 등)도 연속 반복 금지.

**본문 구조 (8~10문장 엄수)**:
```
① 이번 주 트리거 — 실제로 일어난 일 1문장 ("이번 주 ~가 ~하면서...")
② 핵심 질문 제기 또는 메커니즘 한 줄 요약 ("왜 그럴까요? 핵심은 ~입니다.")
③④ 인과 사슬 구체 설명 2~3문장 (단계별로 이어지게)
⑤ 구체 수치 또는 역사적 사례 1문장 ("예를 들어 ~일 때 ~은 ~였습니다.")
⑥ 대조 예시 1문장 — 그렇지 않은 자산 유형 ("반면 ~는 이 영향이 적습니다.")
⑦ 예외·뉘앙스 1문장 ("단, 항상 이런 건 아니고 ~일 때는...")
⑧⑨ 한국 투자자 관점의 현재 의미 1~2문장 ("그래서 ~가 나오는 날이면...")
```

**톤**: 친구에게 설명하듯. 전문 용어는 바로 풀어씀. "할인율" 같은 용어는 쓰되 다음 문장에서 바로 설명.

**금지**:
- 친구톡 summary/changes에 weekly-insight 내용 언급 금지 (완전 분리)
- 지난 주와 동일한 주제 반복 금지 (`week` 필드로 확인)
- 예측·투자 조언 금지 ("지금 사야 한다" 류)
- 이번 주 트리거 없이 교과서적 주제만 선택하는 것 금지 — 반드시 "왜 이번 주인가"가 있어야 함

**수요일이 아닌 날**: `data/weekly-insight.js` 건드리지 않는다. 파일이 없어도 사이트는 정상 동작(섹션 hidden).

**커밋**: 다른 데이터 갱신과 같은 커밋에 포함 (`git add data/weekly-insight.js`). 작성 실패 시 이 파일만 빼고 커밋.
