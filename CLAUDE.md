...
- **자동 갱신 원격 에이전트 (sandbox)**: 사내 피드 `prices/latest.json`은 `ytvcgoldauysvnqckzze.supabase.co` 호스트라 **sandbox proxy에서 차단됨 → curl 불가**. 대신 **이미 돌리는 웹 검색으로** 대표 종목 현재가·등락률·방향을 확인한다 (`삼성전자 주가`·`SK하이닉스 현재가`·`TICKER stock price` 등). summary 쓰기 직전, 검색으로 잡힌 최신 시세와 **부호(상승/하락)가 일치하는지** 반드시 확인.
...
**사고 이력**: 2026-07-14 kr summary 첫 줄이 `삼성전자 NXT 프리마켓 288,500원 +3.78%·SK하이닉스 2,306,000원 +5.49% 블랙먼데이 후 기술적 반등`으로 나갔으나, 라이브 피드는 삼성전자 254,500원 **-10.7%**·SK하이닉스 1,845,000원 **-15.4%** (블랙먼데이 여진 지속). 자동 갱신 에이전트가 프리마켓 반등을 손으로 써 실제 급락과 정반대·SK하이닉스는 25% 과대. 친구톡 발송 후 사용자 지적으로 발견. 이후 피드 대조 룰 강제.
