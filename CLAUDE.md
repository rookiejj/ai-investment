...
`summary`는 매일 평일 KST 08:00 친구톡에 **한 탭 한 줄(글머리표 1개)로 박힌다**. daily-send은 summary 텍스트 그대로 출력한다 (자르거나 분할하지 않음).

- **친구톡 1000자 한도**: 6탭 합산. 초과 시 daily-send이 마지막 탭 블록부터 자동 트림하지만, 그러기 전에 작성자가 절제하는 것이 우선.
- **한 탭 summary 권장 길이**: ~150자 이내. 6탭 × 150자 ≈ 900자로 한도 내 마진 확보.
