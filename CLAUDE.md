...
- 안전망: `scripts/cartoon/headline-dedup.js`가 렌더 단계에서 탭 간 첫 줄 중복을 감지해 다음 줄로 자동 대체한다. **단 이건 폴백이지 면죄부가 아니다** — 에이전트가 애초에 첫 줄을 분산시켜야 의도한 헤드라인이 1면에 나온다(dedup이 대체하면 2번째 줄이 헤드라인으로 승격돼 의도와 달라짐).
