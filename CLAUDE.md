...
`summary`·`rs`·`detail`에 **개별 종목의 "현재가·등락률"을 적을 때는 반드시 사이트 라이브 피드 `prices/latest.json`과 대조해 일치**시킨다. 웹 검색·기억으로 쓴 값과 피드가 벌어지면 **피드가 authoritative** — 피드 값으로 교체한다.

```bash
# 대표 종목 현재가·등락률 확인 (kr=6자리 코드, us=티커)
...
**왜 엄수인가**: 사이트 종목 카드·핀맵은 이 피드를 그대로 표시한다. summary는 친구톡·인스타·헤드라인 hero에 별도로 손으로 쓴 숫자가 나간다. 둘이 벌어지면 사용자가 **같은 종목인데 카드는 254,500원 하락, 친구톡은 288,500원 상승**처럼 정면충돌을 본다. 특히 급락·급등(블랙먼데이 류)일수록 검색 값·프리마켓 추정치가 실제 개장 방향과 반대로 적히기 쉽다.

**규칙**:
- 대표 종목(각 탭 summary 첫 줄에 등장하는 종목)의 **가격·등락률·방향(±)**은 편집 직전 피드로 최종 검증. 피드 대비 **±3% 이상 벌어지거나 부호(상승/하락)가 반대면 피드 우선**으로 교체.
- 피드에 없는 종목(비상장·해외·신규 상장 첫날 등)은 검색 값 사용하되 출처 명확할 때만.
- **방향이 핵심** — 피드가 하락인데 summary가 "반등·상승"으로 프레이밍되면 숫자만 고치지 말고 서술 톤(반등→여진·낙폭 확대 등)도 피드 방향에 맞춘다.
