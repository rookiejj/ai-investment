...
### update.js 트리밍 — 커밋 직전 entries 5건 한도 강제 (자동 갱신 필수 단계)
...
5/11 archive 인프라 도입(`scripts/rotate-archive.js` + `archive-rotate.yml`)으로 mentions 깊이는 archive에서 ~30일 보장. main은 화면 노출(`entries[0]`) + archive 사고 시 폴백만 담당하면 충분. **byte 한도 → entries 5건 한도**로 단순화.

**룰**: update.js 이력 prepend 후 커밋 직전에 다음 명령 실행:

```bash
node scripts/trim-update-logs.js   # 모든 *-update.js를 최대 5건으로 자동 트리밍
```

- 5건은 **상한**(최대). 짧은 탭(kr·commodity 4건 등)은 그대로 유지 — 강제로 5건 채우지 않음
- 잘려나간 entries는 `data/*-update-archive.js`에 archive-rotate.yml이 흡수해 ~30일치 보존
- 결과 항상 멱등 — 이미 5건 이하면 skip

**왜 5건인가**: 친구톡(`PER_TAB=1`)·인스타 캐러셀·TL;DR·hero 모두 `entries[0]`만 사용. 5건 = 약 1주일치(통합 후)로 archive 워크플로 일시 실패 시 main만으로 ~1주일 mentions 폴백 가능. main 사이즈 ~12~15KB로 sandbox push 한계(~50KB) 여유 대폭 증가. 1건까지 줄이면 archive 신뢰성에 100% 의존, 5건은 안전망 비용 ~12KB.
