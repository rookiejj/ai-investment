...
## 🔴 친구톡/탭 문구 "실험·수정" 요청 — 기본은 미리보기(scratch), 프로덕션은 명시적으로만 (엄수)

대화형 세션에서 사용자가 **친구톡·탭 summary 문구를 바꿔달라**고 할 때(예: "톤 바꿔줘", "문구 캐주얼하게", "첫 줄 다르게", "이모지 넣어봐")는 **기본적으로 로컬 미리보기(scratch)로만 처리**한다. 세션이 새로 시작돼 맥락이 없어도 이 기본값을 따른다.

**기본 동작 (의도가 모호하면 항상 이쪽)**:
- 결과를 `scratch/summaries.json`에 쓰고 `node scripts/preview-friendtalk.js`로 보여준다.
- 입력은 `scratch/input/<tab>.md`(고정 팩트). 없으면 현재 `data/<tab>-update.js`에서 얼려 만든다.
- **`data/*-update.js` 편집 금지, `git push` 금지.** scratch는 gitignore돼 프로덕션(인스타·친구톡·DB)에 절대 안 나간다.
- 프롬프트 실험 하니스 설명은 `scratch/README.md`.

**프로덕션 반영은 사용자가 명시할 때만**: "프로덕션 반영", "실제로 반영", "발송되게", "push해", "데이터 파일에 반영" 같은 명시적 지시가 있을 때만 `data/*-update.js`를 편집하고, **push 전 한 번 확인**한다(라이브 친구톡 발송·인스타 직결이라). 모호하면 프로덕션으로 가지 말고 미리보기로.

**예외 — 이 규칙이 적용되지 않는 경우**:
- 자동 갱신 파이프라인(리모트 트리거·`/update*` 스킬): 원래 임무가 `data/*-update.js` 편집 + push라 그대로 수행. 위 "기본 미리보기"는 **대화형 문구 실험 요청**에만 적용.
- 사용자가 처음부터 프로덕션 데이터 갱신을 지시한 경우(위 [자동화 정책]의 "사용자 지시 편집").

**Why**: scratch 미리보기 하니스는 이 대화 맥락을 아는 세션에서만 통했다. 새 세션의 클로드가 "톤 바꿔줘"를 받으면 곧장 `data/*.js`를 고치고 push해 프로덕션 사고를 낼 수 있다. 기본값을 레포(모든 세션이 읽는 CLAUDE.md)에 박아 세션 무관하게 안전하게 만든다.
...
│   ├── daily-insight.js        ← 오늘의 연결고리 (매일 에이전트 생성, 최근 7건 유지, index.html 전용 — 친구톡 미반영)
...
### 🔴 현재가·등락률은 라이브 피드와 대조 (엄수 — 친구톡·헤드라인 직결)

`summary`·`rs`·`detail`에 **개별 종목의 "현재가·등락률"을 적을 때는 반드시 편집 직전에 그 값을 라이브 시세로 교차검증**한다. 기억·이전 세션 값·프리마켓 추정치를 그대로 쓰지 말 것.

**🔴 실행 환경별 검증 소스 (엄수 — 환경에 맞는 소스를 쓸 것)**

- **🔴 자동 갱신 원격 에이전트 (sandbox) — 레포 안 `data/prices-snapshot.json`이 1차 소스 (엄수)**: 사내 피드 `prices/latest.json`은 `ytvcgoldauysvnqckzze.supabase.co` 호스트라 sandbox proxy에서 차단됨 → curl 불가. 그래서 **GitHub Actions(`prices-snapshot.yml`)가 매 거래일 장 마감 직후 실제 종가를 `data/prices-snapshot.json`으로 커밋**해 둔다. sandbox는 이 파일을 git 작업공간에서 그대로 읽어 KR/US **개별 종목 가격·등락률을 쓴다 — 창작·기억·추정 금지**. 형식: `{ asOfKst, session, prices:{ "005930":{c,pc,chg}, ... } }` (c=종가/현재가, pc=직전 종가, chg=등락률%). 읽는 법:
  ```bash
  node -e "const d=require('./data/prices-snapshot.json');console.log(d.asOfKst,d.session);['005930','000660'].forEach(t=>{const x=d.prices[t];console.log(t,x.c,x.chg+'%');})"
  ```
  이 파일에 없는 종목(신규 편입·비상장 등)이나 **지수 레벨(KOSPI 등, 파일 미포함)**만 웹 검색으로 보충하되, 검색값의 **방향(±)이 스냅샷 개별주들의 전반적 방향과 일치**하는지 확인. 스냅샷이 stale해 보이면(asOfKst가 최근 거래일 아님) 그 사실을 근거로 삼아 웹 검색으로 대조.
- **수동·로컬 실행 (`/update-kr` 등, Supabase 접근 가능)**: 사이트 피드가 최종 정답. 아래 한 줄로 대조:
  ```bash
  curl -s "https://ytvcgoldauysvnqckzze.supabase.co/storage/v1/object/public/prices/latest.json?_=$(date +%s)" \
   | node -e "const d=JSON.parse(require('fs').readFileSync(0));const p=d.prices||d;['005930','000660'].forEach(t=>{const x=p[t];console.log(t, x&&x.price, x&&(x.changePct||0).toFixed(2)+'%');})"
  ```

**왜 엄수인가**: 사이트 종목 카드·핀맵은 피드를 그대로 표시한다. summary는 친구톡·인스타·헤드라인 hero에 별도로 손으로 쓴 숫자가 나간다. 둘이 벌어지면 사용자가 **같은 종목인데 카드는 254,500원 하락, 친구톡은 288,500원 상승**처럼 정면충돌을 본다. 특히 급락·급등(블랙먼데이 류)일수록 검색 값·프리마켓 추정치가 실제 개장 방향과 반대로 적히기 쉽다.

**규칙**:
- 대표 종목(각 탭 summary 첫 줄에 등장하는 종목)의 **가격·등락률·방향(±)**은 편집 직전 라이브 시세로 최종 검증. **±3% 이상 벌어지거나 부호(상승/하락)가 반대면 라이브 값 우선**으로 교체.
- **방향이 핵심** — 실제가 하락인데 summary가 "반등·상승"으로 프레이밍되면 숫자만 고치지 말고 서술 톤(반등→여진·낙폭 확대 등)도 방향에 맞춘다. 특히 "블랙먼데이/폭락" 같은 단어를 쓰면서 개별 종목은 +로 적는 자기모순 금지.
- 피드·검색에 안 잡히는 종목(비상장·해외·신규 상장 첫날 등)은 검색 값 사용하되 출처 명확할 때만.
- **🔴 프리마켓·장전엔 "장중 등락률"을 만들지 말 것 (엄수)**: 07시 자동 갱신은 KR 장 개장(09시) 전이라 오늘 장중 데이터가 없다. 이때는 **직전 거래일 종가**(`prices-snapshot.json`의 `c`, session이 "장 마감 후"면 그게 종가)를 쓰고, 오늘자 "+X% 반등/급등/주도" 같은 장중 표현을 지어내지 않는다. 종가 기준임이 드러나게 서술("직전 종가"·"마감 기준")하거나 등락률 없이 레벨만.
- **🔴 거래일 확인 후 서술 (엄수)**: "N월 D일 마감/반등" 처럼 날짜에 장 이벤트를 붙일 땐 그 날짜가 실제 거래일인지 `TZ=Asia/Seoul date`로 요일을 확인. **토·일·휴장일에 "마감·종가·반등"을 부여 금지.** 오늘이 월요일이면 직전 거래일은 금요일(금요일 휴장이면 목요일)임을 계산해 쓴다.

**사고 이력**:
- 2026-07-14 kr summary 첫 줄이 `삼성전자 NXT 프리마켓 288,500원 +3.78%·SK하이닉스 2,306,000원 +5.49% 블랙먼데이 후 기술적 반등`으로 나갔으나, 라이브 피드는 삼성전자 254,500원 **-10.7%**·SK하이닉스 1,845,000원 **-15.4%** (블랙먼데이 여진 지속). 프리마켓 반등을 손으로 써 실제 급락과 정반대.
- 2026-07-20(월) kr 첫 줄 `KOSPI 6,857 +0.73% 블랙먼데이 후 반도체 반등 - SK하이닉스 +3.74%·삼성 +3.63% 주도`, detail엔 `"7/18 반등 마감"`. 실제 마지막 종가는 삼성 255,000·하이닉스 1,842,000(오늘 장중 삼성 0%·하이닉스 +1%)이라 등락률 전부 창작, **7/18은 토요일이라 존재하지 않는 장**을 마감으로 부여, 조선주는 오늘 하락인데 "사상 최대 수혜" 상승 프레이밍. 원인: sandbox 피드 차단 → 웹검색 의존 → 프리마켓 창작. 대응: `prices-snapshot.json`(Actions가 종가 커밋)을 1차 소스로 강제 + 위 프리마켓·거래일 룰 추가.
...
- **🔴 kr 첫 줄은 반드시 한국 시장 주체(한국 종목·지수·정책)여야 한다.** 미국·AI·글로벌 사건은 그것이 한국에 아무리 중요해도 kr 첫 줄에 올리지 않는다 — 첫 줄 충돌이자 cross-tab 인용 금지 위반. 필요하면 한국 주체로 환원한 고유 사건(예: "외국인 반도체 순매수 확대")을 첫 줄로.
- 같은 사건의 *각도만 바꾼* 첫 줄(예: kr "G7 - 이재명 출국" / stocks "G7 - AI 3강 참석" / ai "G7 - AI 3강 정렬")도 충돌로 본다. 1면에서 "G7" 세 줄이 쌓이는 게 핵심 문제. **꼬리말만 자기 탭 도메인으로 바꾸는 것으로는 충돌이 해소되지 않는다** — 예: kr "Oracle 감원 - 한국 반도체 후방" / stocks "Oracle 감원 - SoftBank 재확인" / ai "Oracle 감원 - OpenAI 정당화"는 셋 다 "Oracle 감원"이 주체라 충돌. 각 탭은 **주체 자체가 다른 사건**을 첫 줄로 골라야 한다.
- **🔴 커밋 직전 첫 줄 대조 자기검증 (엄수 — 병렬 작성이라 이 단계 없으면 충돌 못 잡음)**: 5개 탭(kr·stocks·ai·commodity·unicorn) summary 첫 줄을 한자리에 나열하고 기계적으로 확인.
  1. 두 탭 이상의 첫 줄이 **같은 고유명사(기업·자산·인물·사건명)를 2개 이상 공유**하면 동일 사건으로 간주 → 소유 도메인 탭 1개만 남기고 나머지는 자기 도메인 **다른 주체**의 사건으로 교체.
  2. kr 첫 줄에 한국 외 종목·기업명이 등장하면 교체.
  3. 위 둘 통과 못 하면 커밋 금지 — 해당 탭 첫 줄 재작성 후 다시 대조.
- 안전망: `scripts/cartoon/headline-dedup.js`가 렌더 단계에서 dedup. **공유 토큰 3개 이상이면 프레이밍 도메인이 달라도 병합**(꼬리말만 바꾼 동일 사건 도배 차단), 공유 토큰 정확히 2개 + 같은 소유 도메인이면 병합, 배경만 겹치는 다른 사건("G7-이재명 출국"/"G7-AI 3강", 공유 1개)은 각각 유지. **단 이건 폴백이지 면죄부가 아니다** — 병합돼 줄이 제거된 탭은 슬라이드 불릿이 줄어 비어 보인다. 애초에 각 탭이 자기 도메인 고유 사건으로 첫 줄을 채워야 1면·슬라이드가 균형 잡힌다. 매크로 사건(FOMC·지정학)을 kr summary에 과다 적재하지 말 것.

**사고 이력**:
- 2026-06-15 kr·stocks·ai 세 탭이 전부 "G7 에비앙 개막"을 첫 줄로 올려 1면 상단 3줄 도배. 렌더 dedup 도입으로 구조 차단.
- 2026-07-18 kr·stocks·ai·unicorn 네 탭이 "Oracle 3만 감원·Stargate $500B"를 첫 줄로 올림. 각 탭이 꼬리말만 자기 도메인용(kr 한국 반도체 후방·stocks SoftBank·ai OpenAI·unicorn 프리IPO)으로 바꿔 dedup의 소유 도메인이 갈려 통과 → 1면 5줄 중 4줄 도배, 게다가 kr 첫 줄이 미국 기업(Oracle)이라 cross-tab 위반까지. dedup을 "공유 토큰 3+ 면 도메인 무관 병합"으로 강화하고 위 커밋 직전 첫 줄 대조 자기검증을 1차 방어선으로 추가.
...
- **줄 수**: 탭당 3~5줄 권장.
- **한 줄 길이**: ~30~40자 권장.
- **🔴 줄 구조 = `[주체 + 핵심 숫자] - [쉬운 풀이말]` (엄수 — 가독성 직결)**. 각 줄은 앞에 그 사건을 규정하는 **앵커 숫자 1개**(가격·등락률·규모)를 두고, 대시(`-`) 뒤 풀이말은 **정성 표현으로 쉽게** 푼다.
  - **🔴 줄당 숫자 ≤ 2개 (엄수)**. 곁다리 맥락 숫자(forward P/E·earnings yield·10y gap·bp·컨센 대비 %·YoY % 여러 개)를 한 줄에 쌓지 말 것 — 그런 기관급 수치는 `detail`·`rs`에만 두고 summary에선 뺀다. summary는 친구톡·신문 1면·인스타에 그대로 나가는 면이라 정보 밀도보다 가독성이 우선.
  - **방향·규모는 말로**: "급락·동반 약세·사상 최대·감소 우려·버팀목·분수령" 등. 정확 수치가 그 줄의 핵심일 때만 숫자로.
  - **금액 단위는 한글+통화** (`1,165억달러`·`15조원`·`9,650억달러`). 영어 약어(B·T·M) 노출 금지 — 억/조로 푼다.
  - 이 규칙은 아래 [기관급 톤 가이드]의 렌즈 원칙을 **summary에 한해 조정**한다: 렌즈는 유지하되 **줄당 하나, 가능하면 정성적으로**. detail·rs는 기존대로 수치 밀도 허용.
  - ❌ 나쁜 예 (숫자 9개): `현대차·기아 7/24 컨센 매출 50조·31.9조·OP 3.24조 -9.9% - 미국 Q2 하이브리드 5.9만대 +226% YoY 마진 카운터`
  - ✅ 좋은 예 (숫자 3개): `현대차·기아 -6%대 급락 - 7/24 실적 앞두고 미국 하이브리드 판매가 버팀목`
  - ✅ 좋은 예: `코스피 6,516 -4.5% 급락 - 이번 주 실적 발표가 반등의 분수령` / `앤트로픽 IPO 추진 - 기업가치 9,650억달러, 매출 5배 성장`
...
**작성 예시** (JS template literal 또는 `\n` 이스케이프 둘 다 가능). 🔴 각 줄 `[주체+핵심숫자] - [쉬운 풀이말]`·줄당 숫자 ≤2개·한글 단위. 아래처럼 앞엔 앵커 숫자 1개, 대시 뒤는 정성 표현.
```js
summary: `엔비디아 +4% 사상 최고 - 시총 5조달러 첫 돌파, AI 랠리 주도
S&P500·나스닥 동반 신고가 - 실적 시즌 앞두고 위험선호 회복
퀄컴·오픈AI·미디어텍 협력 - 2028년 AI 스마트폰 칩 공동 개발
이란 호르무즈 제안 미국 전달 - 핵협상은 후순위로 분리`,
```
↓ daily-send 출력 결과
```
🇺🇸 미국 마켓

• 엔비디아 +4% 사상 최고 - 시총 5조달러 첫 돌파, AI 랠리 주도
• S&P500·나스닥 동반 신고가 - 실적 시즌 앞두고 위험선호 회복
• 퀄컴·오픈AI·미디어텍 협력 - 2028년 AI 스마트폰 칩 공동 개발
• 이란 호르무즈 제안 미국 전달 - 핵협상은 후순위로 분리
```
...
**🔴 실적일(earnings)은 자동 수집 — 에이전트가 개별 종목 실적을 fixed에 넣지 말 것 (엄수)**: `scripts/fetch-earnings-calendar.js`(GitHub Actions `earnings-calendar.yml`, 주 2회)가 우리 유니버스(stocks/kr-stocks-data.js 277종목)의 다음 실적일을 Yahoo에서 긁어 **`const autoEarnings` 배열**로 자동 반영한다(`src:'auto-earnings'`, 향후 45일). `calendarEvents`가 `fixed.concat(autoEarnings)`로 합쳐 렌더. 따라서:
- 자동 갱신 에이전트는 **개별 기업 실적 발표일을 `fixed`에 수동 추가하지 않는다** — 중복이고 autoEarnings가 이미 촘촘히 채운다.
- `autoEarnings` 블록·`calendarEvents` 라인은 **기계 관리분이니 에이전트가 손대지 말 것**. 에이전트는 `fixed`만 편집.
- 에이전트의 `fixed` 역할은 **실적 외 비정형 이벤트**로 좁힌다: IPO·펀딩, 신제품·모델 출시, 컨퍼런스(WWDC·GTC 등), 매크로 지표(CPI·FOMC·NFP 등), 휴장·정책 결정. autoEarnings가 못 잡는 것만.

**자동 갱신 시 추가 정책** — 매 실행마다 다음을 자연스럽게 함께 점검:

1. **새 fixed 이벤트 발견 시 추가 (실적 제외)**: 조사 단계에서 향후 60일 안 알려진 **비정형** 일정(예: "6/9 WWDC", IPO 프라이싱, 신제품 출시)을 발견하면 `fixed` 배열 끝에 **append**. 같은 `date+title` 중복 시 무시. **개별 종목 실적일은 autoEarnings가 담당하므로 추가 금지.**
...
**무엇이 fixed에 들어가야 하나** (좋은 후보 — ⚠ 개별 종목 실적일 제외, 그건 autoEarnings 담당):
...
매일 자동 갱신 시 새 항목을 배열 맨 앞에 prepend. **index.html 전용 — 친구톡·일간 업데이트와 완전히 분리.**

**🔴 파이프라인 보호 (엄수)**: 작성에 실패하거나 좋은 주제를 찾지 못하면 **이 단계를 skip하고 메인 데이터 파일만 커밋한다**. 이 파일이 메인 커밋(인스타 포스팅 트리거까지 이어지는 파이프라인)을 절대 막으면 안 된다.

**목적**: 사용자가 "몰랐던 자산 간 인과관계"를 매일 하나씩 배우는 코너. 뉴스 나열이 아니라 **메커니즘** 설명.

**스키마 (배열, 최신순)**:
```js
const DAILY_INSIGHTS = [
  {
    date: "2026-08-02",         // 오늘 날짜 (KST)
...
1. **오늘 움직임이 트리거** — 5탭 데이터 조사 단계에서 발견한 가장 큰 cross-asset 이벤트(지수 급등락·금리 급변·환율 이벤트·원자재 스파이크)를 주제의 씨앗으로 삼는다.
2. **비직관적 연결** — "A가 오르면 B도 오른다" 상관관계가 아니라 "왜?"가 필요한 인과 메커니즘. 독자가 "몰랐네"라고 반응할 만큼 의외여야 한다.
3. **한국 투자자 관련성** — 코스피·코스닥·원달러 환율과 연결지어 생각할 수 있어야 한다.
4. **설명 가능성** — 4~6개 인과 단계로 명확하게 설명 가능한 것. 너무 복잡한 메커니즘은 피한다.
5. **최근 7건 비중복** — `DAILY_INSIGHTS` 배열의 기존 `title`을 확인해 비슷한 메커니즘 연속 반복 금지.

**본문 구조 (4단락, \n\n 구분)**:
```
1단락: 오늘 트리거 — 실제 일어난 일 + 의문 제기 (2문장)
2단락: 핵심 메커니즘 인과 사슬 (3문장)
3단락: 구체 수치/역사 사례 + 대조 예시 (2~3문장)
4단락: 예외·뉘앙스 + 한국 투자자 관점 현재 의미 (2문장)
```

**톤**: 친구에게 설명하듯. 전문 용어는 쓰되 다음 문장에서 바로 풀어씀.

**금지**:
- 친구톡 summary/changes에 daily-insight 내용 언급 금지 (완전 분리)
- 오늘 트리거 없이 교과서적 주제만 선택하는 것 금지
...
const SECTOR_ROTATION = [
  {
    date: "2026-08-02",           // prices-snapshot.json의 asOfKst 날짜
    kr: { "반도체": 22.93, "바이오·제약": 2.33, ... },  // 섹터명: 섹터 내 평균 등락률(%)
    us: { "AI 플랫폼": 0.80, "우주": 3.73, ... },
  },
  // 이전 항목들...
];
```
...
**계산 방법**:
```bash
node scripts/calc-sector-rotation.js
# stdout: { date, kr:{...}, us:{...} } JSON
# stderr: 계산 완료 로그
```
...
**커밋**: 다른 데이터 갱신과 같은 커밋 (`git add data/sector-rotation.js`). 실패 시 이 파일만 빼고 커밋.
...
**🔴 summary에는 이 렌즈를 절제 적용 (엄수)**: 아래 렌즈 예시들은 `rs`·`detail` 기준이다. **`summary`는 [summary 작성 규칙]의 `[주체+숫자]-[쉬운 풀이말]`·줄당 숫자 ≤2개 룰이 우선**한다. 즉 summary 줄에 밸류·매크로 렌즈를 넣더라도 **숫자를 여러 개 쌓지 말고 하나만, 나머지는 정성 표현으로** 푼다. 렌즈의 풍부한 수치(forward P/E·10y gap·bp 등)는 detail·rs에서 살리고 summary에선 뺀다 — summary는 친구톡·신문 1면·인스타에 그대로 노출되는 면이라 밀도보다 가독성.

**공통 4가지 렌즈 (모든 탭 적용)**

🔴 아래 ✅ 예시는 **`rs`·`detail` 전용**이다 (수치 밀도 허용). **summary에는 이 밀도로 쓰지 말 것** — summary는 렌즈 하나를 정성적으로만(각 렌즈 끝의 "summary판" 참고). 헷갈리면 summary는 숫자 덜 쓰는 쪽.

1. **매크로 컨텍스트** — 10y 국채금리·DXY·CPI·연준 정책을 사건 배경으로 인용
   - ❌ "AAPL Q2 매출 $90B 컨센 상회"
   - ✅ rs/detail: "AAPL Q2 매출 $90B 컨센 상회 - 달러 119 강세 환율 헤드윈드 30bp 흡수"
   - ✅ summary판: "애플 Q2 컨센 상회 - 강달러 환율 부담 딛고 선방"

2. **밸류에이션 컨텍스트** — forward P/E·earnings yield·sector relative·peer multiple
   - ❌ "NVDA $216 +4% 신고가"
   - ✅ rs/detail: "NVDA $216 +4% forward P/E 32 sector premium 유지·earnings yield 3.1% 10y gap 150bp"
   - ✅ summary판: "엔비디아 216달러 +4% 신고가 - 밸류 부담 딛고 프리미엄 유지"

3. **cross-asset 시그널** — 채권 spread·FX 모멘텀·원자재 ratio·VIX
   - ❌ "한국 10년물 4.2% 부담"
   - ✅ rs/detail: "한국 10년물 4.2%·美 10y gap 40bp 외국인 채권 비중 카운터"
   - ✅ summary판: "한국 국채금리 상승 - 한미 금리차가 외국인 수급 변수"

4. **정책 환경** — Fed funds path·중앙은행 변곡·재정 정책
   - ❌ "FOMC 매파"
   - ✅ rs/detail: "Fed funds 3.6% 동결·dot plot 25bp 인하 1회로 후퇴·BOJ YCC 정상화 6월 분기점"
   - ✅ summary판: "FOMC 금리 동결 - 연내 인하 기대는 한 차례로 후퇴"
